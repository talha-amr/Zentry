...
const Hero = () => {
  ...
  useGSAP(() => {
    if (hasClicked) {
      setIsTransitioning(true); // animation started
      ...
      gsap.to("#next-video", {
        ...
        onComplete: () => {
          setIsTransitioning(false); // animation finished
          setHasClicked(false);
        },
      });
    ...
    <section id='hero' className='h-dvh w-screen relative overflow-x-hidden'>
      ...
      <div
        ...
        className={`origin-center scale-50
          ${isTransitioning
              ? 'opacity-0 pointer-events-none transition-none' // sudden opacity change
              : 'opacity-0 pointer-events-auto transition-all duration-500 ease-in hover:scale-100 hover:opacity-100'} // smooth fade
        `}
        id="minivideo"
      >
            <video
